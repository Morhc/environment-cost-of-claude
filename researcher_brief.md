...
Claude Code writes a local transcript for every session, and each assistant message carries the token counters the serving stack returned: fresh input, cache writes, cache reads, and output. Those are exactly the four categories the Couch rate-set prices separately. Aggregating them over **104 sessions and 50,995 assistant messages** spanning 58 days (15 June to 12 August 2026) and belonging to one heavy user, a working researcher, across scientific-computing, data-analysis, and writing projects, gives the following. The sessions come from two machines, a laptop and an HPC login node, collected separately and merged. There is no central ledger to read them from. Only the counters were read; no message content was examined.

Table: Measured token and energy mix — 104 sessions across two machines (a laptop and an HPC login node), subagent transcripts included. Token counts are measured; the energy column applies the Couch rates and is therefore derived.

| | Tokens | Share of tokens | Energy at Couch rates | Share of energy |
|:---|---:|---:|---:|---:|
| Fresh input (incl. cache writes) | 402.4 M | 2.9% | 156.9 kWh | 19.6% |
| Cached reads | 13,657.3 M | 96.7% | 532.6 kWh | **66.6%** |
...
**Caching is effectively always on.** The measured hit rate is **98.1%**. Section 2 treated caching as the single largest uncertainty and showed both an on and an off case. The off case is a bound that ordinary use does not approach. This is a property of the serving stack rather than of the workload, so it generalises better than anything else in this section.

**The output share is 19–24%, not 50%.** This brief previously swept 20–80% and centred on 50% because nobody had measured it. Measured on the laptop's main conversation threads — the closest analogue to the modelled scenario — output is 19.0% of non-cached tokens, or 23.5% if cache-writes are attributed to new content instead. The full two-machine aggregate is lower still, 12.4%, because the cluster work is agentic and its "input" is dominated by tool results. The scenario keeps 19–24% as the conversational figure; a reader whose work looks like the cluster half should expect to sit lower again. Since output tokens carry ~5× the per-token energy of fresh input, that correction alone lowers every headline figure by a factor of **1.4**, which is why the numbers in this revision are lower than in the previous one.
